...
export default function Products(id: string) {
  ...
  const filterbyMonth = async () => {
    // Declare variables outside the if-block for wider scope
    let isoStartdate: string | undefined;
    let isoEnddate: string | undefined;
  
    if (startMonth && endMonth) {
      // Assign converted dates
      isoStartdate = new Date(startMonth).toISOString();
      isoEnddate = new Date(endMonth).toISOString();
    }
  
    console.log(isoStartdate);
    console.log(isoEnddate);
  
    if (isoStartdate && isoEnddate) { // Ensure dates are valid before calling fetch
      const res = await fetch(
      ...
      if (res.status === 200) {
        ...
        console.log("Error occurred while filtering the products by month");
      }
    } else {
      console.log("Start or end month is missing");
    }
  };
